...
const CESFAM=[
  'todos','CESFAM Karol Wojtyla','CESFAM Padre Manuel Villaseca','CESFAM Vista Hermosa','CESFAM Cardenal Raul Silva Henriquez','CESFAM San Gerónimo','CESFAM Laurita Vicuña','CESFAM Alejandro del Río','CESFAM Bernardo Leighton'
];
const currentFilters={estado:'todos',prioridad:'todos',cesfam:'todos',fecha:'todos',busqueda:''};
...
function applyCurrentFilters(){
  filteredSolicitudesData=solicitudesData.filter(solicitud=>{
    ...
    if(currentFilters.prioridad!=='todos'&&(solicitud.prioridad||'').toLowerCase()!==currentFilters.prioridad)return false;
    ...
    if(currentFilters.fecha!=='todos'){
      ...
      switch(currentFilters.fecha){
        case 'hoy':if(!isSameDay(solicitudDate,today))return false;break;
        case 'semana':case 'esta semana':const weekAgo=new Date(today.getTime()-7*24*60*60*1000);if(solicitudDate<weekAgo)return false;break;
        case 'mes':case 'este mes':case 'este_mes':const monthAgo=new Date(today.getTime()-30*24*60*60*1000);if(solicitudDate<monthAgo)return false;break;
      }
    }
...
document.addEventListener('DOMContentLoaded',()=>{
  fillSelectOptions('filtro-estado-solicitudes',ESTADOS,{
    todos:'Todos los estados',agendado:'Agendado/a',pendiente:'Pendiente',respondido:'Respondido'
  });
  fillSelectOptions('filtro-prioridad-solicitudes',PRIORIDADES,{
    todos:'Todas las prioridades',baja:'Baja',media:'Media',alta:'Alta'
  });
  fillSelectOptions('filtro-cesfam-solicitudes',CESFAM,{todos:'Todos los CESFAM'});
  applyCurrentFilters();
  document.getElementById('filtro-estado-solicitudes').addEventListener('change',function(e){
    currentFilters.estado=e.target.value;applyCurrentFilters();
  });
  document.getElementById('filtro-prioridad-solicitudes').addEventListener('change',function(e){
    currentFilters.prioridad=e.target.value;applyCurrentFilters();
  });
  document.getElementById('filtro-cesfam-solicitudes').addEventListener('change',function(e){
    currentFilters.cesfam=e.target.value;applyCurrentFilters();
  });
  document.getElementById('filtro-fecha-solicitudes').addEventListener('change',function(e){
    currentFilters.fecha=e.target.value;applyCurrentFilters();
  });
  document.getElementById('buscar-solicitudes').addEventListener('input',function(e){
    currentFilters.busqueda=e.target.value;applyCurrentFilters();
  });
